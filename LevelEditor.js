class LevelEditor {
	constructor(owner, size = 20) {
		...
		this.schematic = [];
		this.inputs = [];
		this.blocks = [this.basicBlock, this.wall, this.tunnelHorizontal, this.tunnelVertical, this.intersection, this.blackBg, this.entrace, this.ladder, this.floor, this.vent, this.elevator, this.elevButton, this.ventBlock, this.pathfindingPoint, this.door1, this.door2, this.doorTerminal, this.landingPad, this.cellSelector, this.cellBox];
	...
	placeSchematic(schematic){
		...
		for (let c in schematic){
			if (schematic[c][schematic[c].length - 1] === 3 ||schematic[c][schematic[c].length - 1] === 2 ||schematic[c][schematic[c].length - 1] === 1 || schematic[c][schematic[c].length - 1] === 7 || schematic[c][schematic[c].length - 1] === 9 || schematic[c][schematic[c].length - 1] === 10 || schematic[c][schematic[c].length - 1] === 11){
	...
	cellBox(x1, y1, x2, y2){
		...
		y2 = Math.max(y1, y2);
		y1 = a;
		a = new CellBox(x1 * this.size, (x2 + 1) * this.size, y1 * this.size, (y2 + 1) * this.size, this.selectedCell);
	}
}


class GridParticle extends Particle {
	constructor(ent = player, scale = 20, bright = false) {
		...
		if (back){
			...
			a.tickPlaceholderMain = function(){
				if (this.touchSpecific(player)){
					...
					if (euclidianDistance(this.x, this.y, baseBackend.cart.x, baseBackend.cart.y < 30)){
